...
io.on('connection', (socket: Socket) => {
  ...
  socket.on('claim-host', (data?: { songId: string; currentTime: number; isPlaying: boolean }) => {
    ...
    // Update the room state with the new host's current playback status
    if (data && data.songId) {
      room.songId = data.songId;
      room.currentTime = data.currentTime;
      ...
      console.log(`[Server] User ${username} claimed host role (no initial state sent).`);
    }
    ...
    // Broadcast the new host's active state to all other clients in the room immediately
    if (data && data.songId) {
      socket.to(roomId).emit('remote-playback-command', {
